...
int main()
{
	...
	auto aLoad = builder.CreateLoad(TYPE32,aAlloca,"a");
	// i = 0
	auto iAlloca = builder.CreateAlloca(TYPE32);
	builder.CreateStore(CONST(0), iAlloca);
	auto iLoad = builder.CreateLoad(TYPE32, iAlloca,"i");
	// compare i<10?
	...
	// i = i + 1
	iLoad = builder.CreateLoad(TYPE32, iAlloca, "i");
	auto iNew = builder.CreateNSWAdd(iLoad, CONST(1));
	builder.CreateStore(iNew, iAlloca);
	// a = a + i
	aLoad = builder.CreateLoad(TYPE32, aAlloca,"a");
